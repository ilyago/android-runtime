...
{
	auto key = ConvertToV8String(isolate, "tns::MetadataKey");
...
Local<Object> MetadataNode::CreateExtendedJSWrapper(Isolate *isolate, ObjectManager *objectManager, const string& proxyClassName)
{
	...
	if (cacheData.node != nullptr)
	{
		...
		extInstance->SetPrototype(extdCtorFunc->Get(ConvertToV8String(isolate, "prototype")));
...
Local<Object> MetadataNode::CreateJSWrapper(Isolate *isolate, ObjectManager *objectManager)
{
	...
	else
	{
		...
		if (!obj.IsEmpty())
		{
			...
			obj->Set(ConvertToV8String(isolate, "constructor"), ctorFunc);
			obj->SetPrototype(ctorFunc->Get(ConvertToV8String(isolate, "prototype")));
...
Local<Object> MetadataNode::CreateArrayWrapper(Isolate *isolate)
{
	...
	arr->SetPrototype(objPrototype->Get(ConvertToV8String(isolate, "prototype")));
	arr->SetAccessor(ConvertToV8String(isolate, "length"), ArrayLengthGetterCallack, nullptr, Local<Value>(), AccessControl::ALL_CAN_READ, PropertyAttribute::DontDelete);
...
Local<Object> MetadataNode::CreatePackageObject(Isolate *isolate)
{
	...
	if (ptrChildren != nullptr) {
		...
		for (auto childNode: children) {
            packageObj->SetAccessor(ctx, ConvertToV8String(isolate, childNode->name),
...
void MetadataNode::SetClassAccessor(Local<Function>& ctorFunction)
{
	...
	auto classFieldName = ConvertToV8String(isolate, "class");
...
void MetadataNode::SuperAccessorGetterCallback(Local<String> property, const PropertyCallbackInfo<Value>& info)
{
	try
	{
		auto thiz = info.This();
		auto isolate = info.GetIsolate();
		auto key = ConvertToV8String(isolate, "supervalue");
		...
		if (superValue.IsEmpty())
		{
			...
			bool d = superValue->Delete(V8StringConstants::GetToString(isolate));
			d = superValue->Delete(V8StringConstants::GetValueOf(isolate));
...
void MetadataNode::SetInstanceMembersFromStaticMetadata(Isolate *isolate, Local<FunctionTemplate>& ctorFuncTemplate, Local<ObjectTemplate>& prototypeTemplate, vector<MethodCallbackData*>& instanceMethodsCallbackData, const vector<MethodCallbackData*>& baseInstanceMethodsCallbackData, MetadataTreeNode *treeNode)
{
	...
	for (auto i = 0; i < instanceMethodCout; i++)
	{
		...
		if (entry.name != lastMethodName)
		{
			...
			auto funcName = ConvertToV8String(isolate, entry.name);
	...
	for (auto i = 0; i < instanceFieldCout; i++)
	{
		...
		auto fieldName = ConvertToV8String(isolate, entry.name);
...
void MetadataNode::InnerClassConstructorCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	try
	{
		...
		SetInstanceMetadata(isolate, thiz, data->node);
...
void MetadataNode::InnerClassAccessorGetterCallback(Local<String> property, const PropertyCallbackInfo<Value>& info)
{
	try
	{
		...
		auto innerKey = ConvertToV8String(isolate, "inner:" + node->m_treeNode->name);
		...
		if (innerTypeCtorFunc.IsEmpty())
		{
			...
			auto prototypeName = ConvertToV8String(isolate, "prototype");
...
void MetadataNode::SetInnnerTypes(Isolate *isolate, Local<Function>& ctorFunction, MetadataTreeNode *treeNode)
{
	if (treeNode->children != nullptr)
	{
		...
		auto prototypeTemplate2 = ctorFunction->Get(ConvertToV8String(isolate, "prototype")).As<Object>();
		...
		for (auto curChild : children)
		{
			...
			if (isStatic)
			{
				...
				auto innerTypeName = ConvertToV8String(isolate, curChild->name);
...
void MetadataNode::ExtendedClassConstructorCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	try
	{
        ...
        V8SetPrivateValue(isolate, thiz, ConvertToV8String(isolate, "t::implObj"), implementationObject);
...
Local<Object> MetadataNode::GetImplementationObject(Isolate* isolate, const Local<Object>& object)
{
    ...
    Local<Value> hiddenVal;
    V8GetPrivateValue(isolate, object, ConvertToV8String(isolate, "t::implObj"), hiddenVal);
...
void MetadataNode::ExtendCallMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info)
{
	try
	{
		...
		implementationObject->SetAccessor(ConvertToV8String(isolate, "super"), SuperAccessorGetterCallback, nullptr, implementationObject);
...
{
	auto isolate = value->GetIsolate();
	auto node = GetInstanceMetadata(isolate, value);
	return node;
}
...
void MetadataNode::CreateTopLevelNamespaces(Isolate *isolate, const Local<Object>& global)
{
	...
	for (auto treeNode : children)
	{
		...
		if (nodeType == MetadataTreeNode::PACKAGE)
		{
			...
			global->Set(ConvertToV8String(isolate, node->m_treeNode->name), packageObj);
		}
	}
}
...
Local<Function> MetadataNode::Wrap(Isolate* isolate, const Local<Function>& f, const string& name, const string& origin,  bool isCtorFunc)
{
	...
	auto str = ss.str();
	auto source = ConvertToV8String(isolate, str);
	auto context = isolate->GetCurrentContext();

	TryCatch tc;

	Local<Script> script;
	ScriptOrigin jsOrigin(ConvertToV8String(isolate, origin));
	...
	if (!script.IsEmpty())
	{
		...
		if (!result.IsEmpty())
		{
			...
			ret->Set(ConvertToV8String(isolate, "__func"), f);

			auto prototypePropName = ConvertToV8String(isolate, "prototype");
